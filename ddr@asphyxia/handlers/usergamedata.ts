import { CommonOffset, LastOffset, OptionOffset, Profile, RivalOffset } from "../models/profile";
...
const rivalload = async (refId: string, data: any) => {
  const loadFlag = $(data).number("data.loadflag");
  const record = [];

  const profile = await DB.FindOne<Profile>(refId, { collection: "profile" });
  const rivalData = profile?.usergamedata?.RIVAL?.strdata?.split(",") ?? [];

  for (let slot = 0; slot < 3; slot++) {
    const activeIndex = RivalOffset.RIVAL_1_ACTIVE + slot;
    const codeIndex = RivalOffset.RIVAL_1_DDRCODE + slot;

    const isActive = rivalData[activeIndex] === "1";
    const codeValue = parseInt(rivalData[codeIndex] ?? "0", 10) || 0;

    const entry: any = {
      slot: K.ITEM("s32", slot),
      no: K.ITEM("s32", slot + 1),
      is_active: K.ITEM("bool", isActive),
      exists: K.ITEM("bool", false),
      code: K.ITEM("s32", codeValue),
      ddrcode: K.ITEM("s32", codeValue),
      code_disp: K.ITEM("str", codeValue ? formatCode(codeValue) : ""),
      seq: K.ITEM("str", codeValue ? formatCode(codeValue) : ""),
    };

    let rivalProfile: Profile | null = null;

    if (codeValue) {
      rivalProfile = await DB.FindOne<Profile>(null, {
        collection: "profile",
        ddrCode: codeValue
      });
    }
    ...
    if (rivalProfile) {
      const common = rivalProfile.usergamedata?.COMMON?.strdata?.split(",") ?? [];

      entry.exists = K.ITEM("bool", true);
      entry.name = K.ITEM("str", common[CommonOffset.NAME] || "");
      entry.area = K.ITEM("str", common[CommonOffset.AREA] || rivalProfile.shopArea || "");
      entry.shoparea = K.ITEM("str", rivalProfile.shopArea || "");
      entry.single_grade = K.ITEM("u32", rivalProfile.singleGrade || 0);
      entry.double_grade = K.ITEM("u32", rivalProfile.doubleGrade || 0);
    } else {
      entry.name = K.ITEM("str", "");
      entry.area = K.ITEM("str", "");
      entry.shoparea = K.ITEM("str", "");
      entry.single_grade = K.ITEM("u32", 0);
      entry.double_grade = K.ITEM("u32", 0);
    }
  ...
  if (profile?.ddrCode) {
    responseData.myddrcode = K.ITEM("s32", profile.ddrCode);
    responseData.myddrcode_disp = K.ITEM("str", formatCode(profile.ddrCode));
  }
